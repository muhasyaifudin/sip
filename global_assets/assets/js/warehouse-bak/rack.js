var kategoriRak = function () {
	var _componentkategoriRak = function () {
		...
		var initKategoriRakTable = function () {
			racks_tables = $('#racks_tables').DataTable({
				destroy: true,
				processing: true,
				ajax: {
					type: "GET",
					url: qualifyURL("api/warehouse/rack"),
					data: {
					}
				},
				columns: [
				...
				},		    
				{
					...
					render : function(data, type, full, meta) {
						...
						return '<td class="text-center"> <div class="list-icons"> <div class="dropdown"> <a href="#" class="list-icons-item" data-toggle="dropdown"> <i class="icon-menu9"></i> </a> <div class="dropdown-menu dropdown-menu-right"> <a href="api/warehouse/rack" full_data="'+full_data+'" class="dropdown-item rack_edit"><i class="icon-pencil"></i> Edit</a> <a href="api/warehouse/rack/'+data+'" title="" class="dropdown-item rack_dell" ><i class="icon-bin"></i> Hapus</a> </div> </div> </div> </td> </tr>'
					}
				}
				]
			})
			...
			// Record Numbering
			racks_tables.on( 'order.dt search.dt', function () {
				racks_tables.column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
					cell.innerHTML = i+1
				} )
			} ).draw()


			$('#racks_tables').on('click', '.rack_edit', function(event) {
				event.preventDefault();
				rack = JSON.parse($(this).attr('full_data').replace(/'/g, '"'));

				$.each(rack, function(index, val) {
					$('#form_rack').find('input[name="'+index+'"]').val(val)
				});
		...
		$('#racks_tables').on('click', '.rack_dell', function(event) {
			event.preventDefault();
			url = $(this).attr('href');
		...
		$('#form_rack').submit(function(event) {
			event.preventDefault();
